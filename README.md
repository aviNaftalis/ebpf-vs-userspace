...
| pipe \| grep (userspace) | 127 | 1575 | 1.6× | yes |
| strace / ptrace (userspace) | 13586 | 15 | 172.0× | yes |

**Per-`write()` overhead (the honest number):** the bare baseline write costs ~395 ns. eBPF adds **~610 ns** (prefix check) — or just **~530 ns** if it only counts (skipping the user-memory read). The big ×'s above are only because the baseline write (to `/dev/null`) is nearly free; on a process doing real work per syscall, ~610 ns is well under 1%.
